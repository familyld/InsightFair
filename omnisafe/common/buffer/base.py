...
class BaseBuffer(ABC):
    ...
    +--------+---------------------------+---------------+-----------------------------------+
    | reward | (size, \*rew_space.shape) | torch.float32 | Single step reward.               |
    +--------+---------------------------+---------------+-----------------------------------+
    ...
    Args:
        ...
        rew_space (OmnisafeSpace): The reward space.
    ...
    def __init__(
        ...
        if isinstance(act_space, Box):
            ...
            raise NotImplementedError
        if isinstance(rew_space, Box):
            rew_buf = torch.zeros((size, *rew_space.shape), dtype=torch.float32, device=device)
        else:
            raise NotImplementedError
    ...
    def store(self, **data: torch.Tensor) -> None:
        ...
        Examples:
            >>> buffer = BaseBuffer(...)
            >>> buffer.store(obs=obs, act=act, reward=reward, done=done)
