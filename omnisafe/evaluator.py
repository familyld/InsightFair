...
from omnisafe.envs.core import MDP, make
...
class Evaluator:  # pylint: disable=too-many-instance-attributes
    ...
    Args:
        env (MDP or None, optional): The environment. Defaults to None.
    ...
    def __load_model_and_env(
        ...
        if self._env.need_time_limit_wrapper:
            self._env = TimeLimit(self._env, device=torch.device('cpu'), time_limit=20)
        ...
        if hasattr(self._cfgs, 'algo') and self._cfgs['algo'] in [
            ...
            if self._cfgs['algo'] in ['CCEPETS', 'RCEPETS']:
                algo_to_planner = {
                    'CCEPETS': (
                        'CCEPlanner',
