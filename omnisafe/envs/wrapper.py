...
from omnisafe.envs.core import MDP, Wrapper
...
class TimeLimit(Wrapper):
    ...
    Args:
        env (MDP): The environment to wrap.
    ...
    def __init__(self, env: MDP, time_limit: int, device: torch.device) -> None:
...
class AutoReset(Wrapper):
    ...
    def __init__(self, env: MDP, device: torch.device) -> None:
...
class ObsNormalize(Wrapper):
    ...
    def __init__(self, env: MDP, device: torch.device, norm: Normalizer | None = None) -> None:
    ...
    def save(self) -> dict[str, torch.nn.Module]:
        ...
        .. note::
            ...
            and reward_normalize. When evaluating the saved model, the normalizer
            should be loaded.
...
class RewardNormalize(Wrapper):
    ...
    def __init__(self, env: MDP, device: torch.device, norm: Normalizer | None = None) -> None:
        ...
        if norm is not None:
            ...
            self._reward_normalizer = Normalizer((self.reward_space.shape), clip=5).to(self._device)
...
class ActionRepeat(Wrapper):
    ...
    def step(
        ...
        Returns:
            ...
            info: Some information logged by the environment.
        """
        # rewards = torch.tensor(0.0).to(self._device)
        rewards = 0.0
...
class Unsqueeze(Wrapper):
    """Unsqueeze the observation, reward, terminated, truncated and info.
    ...
    def step(
        ...
        obs, reward, terminated, truncated = (
            x.unsqueeze(0) for x in (obs, reward, terminated, truncated)
        )
