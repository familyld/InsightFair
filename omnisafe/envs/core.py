...
class MDP(ABC):
    ...
    The MDP class is the core class of the environment. It defines the basic interface of the
    ...
    def __init__(self, env_id: str, **kwargs: Any) -> None:
        """Initialize an instance of :class:`MDP`."""
        assert (
...
class Wrapper(MDP):
    ...
    Args:
        env (MDP): The environment.
        device (torch.device): The device to use. Defaults to ``torch.device('cpu')``.

    Attributes:
        _env (MDP): The environment.
    """

    def __init__(self, env: MDP, device: torch.device = DEVICE_CPU) -> None:
...
class EnvRegister:
    ...
    def __init__(self) -> None:
        ...
        self._class: dict[str, type[MDP]]
        self._support_envs: dict[str, list[str]]
        self._class = {}
        self._support_envs = {}

    def _register(self, env_class: type[MDP]) -> None:
        """Register the environment class.

        Args:
            env_class (type[MDP]): The environment class.
        """
        if not inspect.isclass(env_class):
            raise TypeError(f'{env_class} must be a class')
        class_name = env_class.__name__
        if not issubclass(env_class, MDP):
            raise TypeError(f'{class_name} must be subclass of MDP')
    ...
    def register(self, env_class: type[MDP]) -> type[MDP]:
        """Register the environment class.
    ...
    def get_class(self, env_id: str, class_name: str | None) -> type[MDP]:
...
def make(env_id: str, class_name: str | None = None, **kwargs: Any) -> MDP:
