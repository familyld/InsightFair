...
class EnsembleModel(nn.Module):
    ...
    def __init__(
        ...
        self._embedding: nn.Embedding = nn.Embedding(group_size, emb_size)
        self._nn1: EnsembleFC = EnsembleFC(
            emb_size + state_size + action_size,
            hidden_size,
            ensemble_size,
    ...
    def forward(
        ...
        """Compute next state, reward using all models.
        ...
        Returns:
            mean: Mean of the next state, reward.
            logvar or var: Log variance of the next state, reward.
        """
        ...
        data_t = torch.cat([emb, data_t], dim=-1)

        nn1_output = swish(self._nn1(data_t))
        nn2_output = swish(self._nn2(nn1_output))
        nn3_output = self._nn3(nn2_output)
        mean = nn3_output[:, :, : self._output_dim]
        logvar = self.max_logvar - F.softplus(
            self.max_logvar - nn3_output[:, :, self._output_dim :],
        )
    ...
    def forward_idx(
        ...
        """Compute next state, reward from an certain model.
        ...
        if isinstance(data, torch.Tensor):
            ...
            data_t = torch.tensor(data, dtype=torch.float32, device=self._device)
        # [group_size, state_size + action_size]
        data_t = self.scaler.transform(data_t[0])
        # [group_size, emb_size]
        emb = emb.squeeze(0)
        # [group_size, emb_size + state_size + action_size]
        data_t = torch.cat([emb, data_t], dim=-1)
        ...
        nn3_output = unbatched_forward_fn(self._nn3, nn2_output)
        mean = nn3_output[:, : self._output_dim]
        logvar = self.max_logvar - F.softplus(self.max_logvar - nn3_output[:, self._output_dim :])
        logvar = self.min_logvar + F.softplus(logvar - self.min_logvar)
        var = torch.exp(logvar)
    ...
    def loss(
        ...
        Args:
            mean (torch.Tensor): Mean of the next state, reward.
            logvar (torch.Tensor): Log variance of the next state, reward.
            labels (torch.Tensor): Ground truth of the next state, reward.
...
class EnsembleDynamicsModel:
    """Dynamics model for predict next state, reward.
    ...
    def __init__(
        ...
        self._emb_size: int = model_cfgs.emb_size
        ...
        self._state_size: int = state_shape[0]//group_size
        self._action_size: int = action_shape[0]//group_size
        self._group_size: int = group_size
    ...
    def train(
        ...
        Args:
            ...
            labels (np.ndarray): Ground truth of the next state, reward.
        ...
        ids = np.arange(self._group_size).repeat(inputs.shape[0]//self._group_size)
        ids, inputs, labels = ids[permutation], inputs[permutation], labels[permutation]

        # split training and testing dataset
        train_ids, train_inputs, train_labels = ids[num_holdout:], inputs[num_holdout:], labels[num_holdout:]
        holdout_ids, holdout_inputs, holdout_labels = ids[:num_holdout], inputs[:num_holdout], labels[:num_holdout]
        ...
        for epoch in itertools.count():
            ...
            for start_pos in range(0, train_inputs.shape[0], self._batch_size):
                ...
                train_id = torch.from_numpy(train_ids[idx]).long().to(self._device)

                train_embed = self._ensemble_model._embedding(train_id)

                mean, logvar = self._ensemble_model.forward(train_embed, train_input, ret_log_var=True)
                total_loss, mse_loss = self._ensemble_model.loss(mean, logvar, train_label)
                self._ensemble_model.train_ensemble(total_loss)
                train_mse_losses.append(mse_loss.detach().cpu().numpy().mean()) # average over batch
            ...
            for start_pos in range(0, holdout_inputs.shape[0], val_batch_size):
                with torch.no_grad():
                    ...
                    val_id = torch.from_numpy(holdout_ids[idx]).long().to(self._device)
                    val_embed = self._ensemble_model._embedding(val_id)
            ...
            current_loss = np.sum(np.array(val_losses_list), axis=0) / ( # average over batch
    ...
    def _predict(
        ...
        # input shape: [num_ensemble, group_size * num_particles * num_samples / num_ensemble, state_shape + action_shape]
        assert inputs.shape[2] == self._state_size + self._action_size

        ids = torch.arange(self._group_size).repeat_interleave(inputs.shape[1]//self._group_size)
        ids = ids[None, :].repeat([inputs.shape[0], 1])

        ensemble_mean, ensemble_var = [], []
        for i in range(0, inputs.shape[1], batch_size):
            # input shape: [num_ensemble, group_size * num_particles * num_samples / num_ensemble, state_shape + action_shape]
            ids_batch = ids[:, i : min(i + batch_size, inputs.shape[1])].long().to(self._device)
            emb_batch = self._ensemble_model._embedding(ids_batch)
        ...
        # shape: [num_ensemble, group_size * num_particles * num_samples / num_ensemble, state_shape + 1], 1 is for reward
    ...
    def sample(
        ...
        if states.shape[1] == actions.shape[1]:
            states = torch.cat(torch.split(states, self._state_size, dim=-1), dim=1)
        actions = torch.cat(torch.split(actions, self._action_size, dim=-1), dim=1)

        # shape: [num_ensemble, group_size * num_particles * num_samples / num_ensemble, state_shape + action_shape]
        ...
        # shape: [num_ensemble, group_size * num_particles * num_samples / num_ensemble, state_shape]
        states = ensemble_samples[:, :, self._state_start_dim :]
        # shape: [num_ensemble, group_size * num_particles * num_samples / num_ensemble, 1]
    ...
    def imagine(
        ...
        """
        assert (
            states.shape[1] == self._state_size * self._group_size
        ...
        if actions is not None:
            assert actions.shape == torch.Size(
                [horizon, states.shape[0], self._action_size * self._group_size],
        ...
        for step in range(horizon):
            # [num_ensemble, group_size * num_particles * num_samples / num_ensemble, action_shape]
            current_actions = get_action(states, step)
            # [num_ensemble, group_size * num_particles * num_samples / num_ensemble, state_shape], [num_ensemble, group_size * num_particles * num_samples / num_ensemble, 1]
            states, rewards, info = self.sample(states, current_actions, idx)
            ...
            traj['actions'].append(current_actions)
    ...
    def imagine_for_fairness_eval(
        ...
        assert actions.shape == torch.Size(
            [states.shape[0], self._action_size * self._group_size],
        ), 'actions should be of shape (batch_size, action_size)'       
  
        traj = defaultdict(list)
        num_ensemble = self._num_ensemble if idx is None else 1
        states = states[None, :, :].repeat([num_ensemble, 1, 1])
        actions = actions[None, :, :].repeat([num_ensemble, 1, 1])

        for _ in range(horizon):
            # split into groups, and only use the first group (disadvantaged group)
            states = torch.split(states, self._state_size, dim=-1)[0]
            actions = torch.split(actions, self._action_size, dim=-1)[0]

            # shape: [num_ensemble, buf_size, state_shape + action_shape]
            inputs = torch.cat((states, actions), dim=-1)

            # use the id of the second group (advantaged group)
            ids = torch.ones(inputs.shape[0], inputs.shape[1]) # [num_ensemble, buf_size]

            ensemble_mean = []
            for i in range(0, inputs.shape[1], batch_size):
                # input shape: [num_ensemble, nubuf_size, state_shape + action_shape]
                ids_batch = ids[:, i : min(i + batch_size, inputs.shape[1])].long().to(self._device)
                emb_batch = self._ensemble_model._embedding(ids_batch)
                model_input = inputs[:, i : min(i + batch_size, inputs.shape[1]), :]

                if idx is None:
                    b_mean, _ = self._ensemble_model.forward(
                        emb_batch,
                        model_input,
                    )
                else:
                    b_mean, _ = self._ensemble_model.forward_idx(
                        emb_batch,
                        model_input,
                        idx,
                    )
                ensemble_mean.append(b_mean)
            # shape: [num_ensemble, buf_size, state_shape + 1], 1 is for reward
            ensemble_mean_tensor = torch.cat(ensemble_mean, dim=1)
            assert (
                ensemble_mean_tensor.shape[:-1] == inputs.shape[:-1]
            ), 'output shape must be the same as input shape except the last dimension'

            ensemble_mean_tensor[:, :, self._state_start_dim :] += states            
            ensemble_samples = ensemble_mean_tensor # deterministic evaluation

            # shape: [num_ensemble, buf_size, state_shape]
            states = ensemble_samples[:, :, self._state_start_dim :]
            # shape: [num_ensemble, buf_size, 1]
            rewards = self._compute_reward(ensemble_samples)
            info = defaultdict(list)
