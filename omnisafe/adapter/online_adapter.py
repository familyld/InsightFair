...
class OnlineAdapter:
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        self._env: MDP = make(env_id, num_envs=num_envs, device=self._device)
        self._eval_env: MDP = make(env_id, num_envs=1, device=self._device)
    ...
    def _wrapper(
        ...
        if self._env.need_time_limit_wrapper:
            self._env = TimeLimit(self._env, time_limit=20, device=self._device)
            self._eval_env = TimeLimit(self._eval_env, time_limit=20, device=self._device)
    ...
    def save(self) -> dict[str, torch.nn.Module]:
        ...
        .. note::
            ...
            ``obs_normalize`` and ``reward_normalize``.
